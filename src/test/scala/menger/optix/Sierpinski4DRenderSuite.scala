...
class Sierpinski4DRenderSuite extends AnyFlatSpec with Matchers with LazyLogging with RendererFixture:
  ...
  "addSierpinski4DInstance" should "return a valid instance ID" in:
    val result = renderer.addSierpinski4DInstance(
      level = 1,
  ...
  it should "return distinct IDs for multiple instances" in:
    val id0 = renderer.addSierpinski4DInstance(
      1, Vector[3](-1.5f, 0f, -5f), scale, eyeW, screenW, 0f, 0f, 0f,
      Material(OPAQUE_RED)
    )
    val id1 = renderer.addSierpinski4DInstance(
      1, Vector[3](1.5f, 0f, -5f), scale, eyeW, screenW, 0f, 0f, 0f,
  ...
  "sierpinski4d rendering" should "produce non-empty image data" in:
