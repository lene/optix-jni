...
class RayStatsSuite extends AnyFlatSpec with Matchers with RendererFixture:
  ...
  it should "have more rays for larger image dimensions" in:
    ...
    val small = renderer.renderWithStats(ImageSize(100, 100)).get
    val large = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
  ...
  it should "have different ray counts for different sphere transparency" in:
    ...
    TestScenario.default()
      ...
      .withHorizontalFOV(60f)
      .applyTo(renderer)
    val semiTransparent = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
    ...
    TestScenario.default()
      ...
      .withHorizontalFOV(60f)
      .applyTo(renderer)
    val moreTransparent = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
  ...
  it should "have consistent stats across multiple renders" in:
    ...
    val result1 = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
    val result2 = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
  ...
  it should "have image data matching render() output" in:
    ...
    val statsRender = renderer.renderWithStats(STANDARD_IMAGE_SIZE).get
